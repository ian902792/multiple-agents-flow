...
以 `--flow quick-codex` 跑同樣兩題，每題 2 次。Codex 走 ChatGPT 訂閱額度，只回報 token、不回報金額：

| 題目 | 一次通過 | 最後通過 | 平均秒數 | 平均總輸入（含快取） | 快取命中 | 平均輸出 | 推理 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| duration | 1/2 | 1/2 | 54 | 114,655 | 84.5% | 1,150 | 0 |
| webapp | 2/2 | 2/2 | 50 | 96,075 | 83.5% | 2,086 | 0 |

`low` 完全不產生推理 token，輸出是三家裡最少的；但每次的總輸入約 8～15 萬 token，是 Pi（約 1.7 萬）的 5 倍以上，主要來自 Codex 自己的系統提示與整份讀檔，其中八成多命中快取。duration 有一次自動修復後仍失敗，最後通過率 3/4，是三家 `low` 裡最低的。
