...
- **多一道 prompt injection 關卡**：repo 裡若藏了想操控 agent 的文字，實作的 coder 與只讀的審查者來自不同家，較不容易被同一段文字同時騙過；加上可修改的檔案被鎖定、coder 不能 commit 或 push、測試由 MAF 自己跑、審查不相信 coder 的說法。這是降低風險而非保證：獨立審查預設關閉，要在 Flow Studio 開啟；工作樹也不是安全沙箱，只在信任的 repo 使用。
